...
export function ProjectCard({ project, translations, viewAll }: { project: Record<string, unknown> & { id: number }; translations: Record<string, string>; viewAll?: boolean }) {
  ...
  return (
    <a href={`/projects/${slug}`} className="group">
      <div className="glass-card rounded-xl overflow-hidden hover:-translate-y-1 transition-all tilt-3d">
        {thumbnail ? (
          <div className="aspect-video overflow-hidden">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={thumbnail} alt={String(project.title)} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
          </div>
        ) : (
        ...
        <div className="p-5">
          <p className="text-xs text-slate-500 mb-1">{String(project.category ?? '')}</p>
          <h3 className="font-semibold text-white group-hover:text-cyan-300 transition-colors">{String(project.title)}</h3>
          <p className="text-sm text-slate-400 mt-2 line-clamp-2">{String(project.description ?? '')}</p>
          {techs.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-3">
              {techs.map((tech) => (
                <span key={tech} className="text-[10px] px-2 py-0.5 rounded-full bg-white/5 text-slate-500">{tech}</span>
              ))}
            </div>
          )}
          {views && (
    ...
    <a href={`/blog/${slug}`} className="group">
      <div className="glass-card rounded-xl overflow-hidden hover:-translate-y-1 transition-all tilt-3d">
        {cover && (
          <div className="aspect-video overflow-hidden">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={cover} alt={String(post.title)} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
          </div>
        )}
        <div className="p-5">
          {category && <p className="text-[10px] uppercase tracking-wider text-slate-500 mb-2">{category}</p>}
          <h3 className="font-semibold text-white group-hover:text-cyan-300 transition-colors">{String(post.title)}</h3>
